...
Eindices = zeros(length(C(:,1)), 1); %--- The indices of the tetrahedrons the points of C are embedded into
for c=1:length(C(:,1))
    Pp = C(c, :)';
    for t=1:length(T(:,1))
        % We don't want to embed multiple via points in the same
        % tetrahedral
        if ~ismember(t, Eindices) 
            i = T(t,1);
            j = T(t,2);
            k = T(t,3);
            m = T(t,4);
...
Bs = zeros(length(C(:,1)), 4); %--- The barycentric coordinates of the cable points
for c = 1:length(C(:,1))
    t = Eindices(c);
    i = T(t,1);
    j = T(t,2);
    ...
    Bs(c, :) = coord2barycentric(Pi, Pj, Pk, Pm, Pp)';
end
